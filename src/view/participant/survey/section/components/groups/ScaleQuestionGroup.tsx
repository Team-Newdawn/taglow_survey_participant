...
import { readLowScoreThreshold, readScaleLabels, readScaleValue, ScaleQuestionBody } from '../questions/ScaleQuestion';
...
type ScaleQuestionGroupProps = {
  ...
  number?: number;
  initialExpandedQuestionId?: string;
...
export function ScaleQuestionGroup(props: ScaleQuestionGroupProps) {
  ...
  useEffect(() => {
    if (!firstMissingQuestionId && props.initialExpandedQuestionId) {
      setExpandedQuestionId(props.initialExpandedQuestionId);
    }
  }, [firstMissingQuestionId, props.initialExpandedQuestionId]);
          ...
          return (
            <div key={question.id} className={`scale-question-group__item${error ? ' has-error' : ''}`}>
              ...
              >
                ...
                <span
                  className={`scale-question-group__toggle${isExpanded ? ' is-expanded' : ''}`}
                  aria-hidden="true"
                />
              </button>
