...
- 명시 brand 이름 (한글/영문) → `.agents/data/reference-fingerprints.json` `items[].id` 매칭
...
1. **homepage URL**:
   ...
   - 둘 다 없으면 `.agents/data/reference-fingerprints.json`의 `items[].category_raw` 기반으로 추론 — 마지막 수단
