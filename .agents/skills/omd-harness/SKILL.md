...
```
omd-master subagent가 이 세션에 등록되어 있지 않아요. Claude Code는 세션 시작 시점에만 .agents/agents/*.md를 로드합니다 — install-skills를 세션 띄운 후에 돌렸으면 이 케이스.

해결 (가장 빠른 순서):
1. 현재 세션에서 /agents 실행 — Claude Code가 .agents/agents/*.md 강제 재스캔. omd-* 8개가 목록에 나타나면 /omd-harness 재호출.
2. define_subagent 툴 호출 및 project root의 agents.md 정의를 확인해 에이전트가 올바르게 로드될 수 있도록 합니다.
```
...
- `.agents/data/reference-fingerprints.json` — reference fingerprint (tone keywords, visual theme, antipatterns, signature motion, has_personas, category)
- `.agents/data/reference-tags.md` — 사람-읽기용 keyword 매트릭스
- `.agents/data/vocabulary.json` — controlled vocab

`.agents/data/`에 없으면 `node_modules/oh-my-design-cli/data/` 또는 패키지 root `data/` 에서 fallback.
